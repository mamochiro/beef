...
func (r *PostgresRepository) FirstOrCreate(filters map[string]interface{}, ent interface{}) error {
	return r.db.Connection.Where(filters).FirstOrCreate(ent).Error
}
...
func (r *PostgresRepository) UpdateByMapInterface(model interface{}, filters map[string]interface{}, fields map[string]interface{}) error {
	result := r.db.Connection.Model(model).Where(filters).Updates(fields)
